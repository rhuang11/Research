...
path = 'C:/Users/d_huang/Documents/PythonProjects/Execucompdata/data/'
...
for filename2 in lst:
    # separation in the data is |, and do not read header becuase there is no header in the data
    dfa= pd.read_csv(path + filename + filename2 + '.csv', header = None)
    dfa = dfa.iloc[1:, 1:]
...
jk1 = jk[['CMTE_ID', 'NAME', 'EMPLOYER', 'TRANSACTION_DT', 'TRANSACTION_AMT', 'CEOANN']]    


# pick the CEO
# jk1 = jk1[(jk1['CEOANN'] == 'CEO')]

# selecting rows based on condition
jk1.dropna(subset = ['TRANSACTION_AMT'], inplace = True)      # Remove rows with NaN
# convert string to numeric
jk1['TRANSACTION_AMT'] = pd.to_numeric(jk1['TRANSACTION_AMT'], errors='coerce')
...
for wyear in range(2000, 2022, 2): 
    print(wyear)
    ...
    # jk8 = outmerge1[(outmerge1['EMPLOYER'] == 'AMERICAN AIRLINES')]
    ...
    # cap last name and first name. 
    outmerge1['NAMEL'] = outmerge1['NAMEL'].str.upper()
    outmerge1['NAMEF'] = outmerge1['NAMEF'].str.upper()
    
    # sum donation to the same committee
    
    
    outmerge2 = outmerge1.groupby(['YEAR','NAME_EXEC', 'NAMEL', 'NAMEF', 'CONAME', 'CONAME_EXEC', 'GVKEY', 'CMTE_ID'])['TRANSACTION_AMT'].sum().reset_index()
    # drop duplicates
    outmerge2 = outmerge2.drop_duplicates(subset=['YEAR', 'NAME_EXEC', 'NAMEL', 'NAMEF', 'CONAME_EXEC', 'CMTE_ID'], keep='first')
    ...
    # the next line reduces the sample. # Remove rows with NaN
...
outmerge5a = outmerge4a.groupby(['YEAR','NAME_EXEC', 'NAMEL', 'NAMEF', 'CONAME_EXEC', 'GVKEY'])['TRANSACTION_AMT'].sum().reset_index()
outmerge5b = outmerge4a.groupby(['YEAR','NAME_EXEC', 'NAMEL', 'NAMEF', 'CONAME_EXEC', 'GVKEY'])['DMR'].sum().reset_index()
...
# jk8 = outmerge4a[(outmerge4a['NAME_EXEC'] == 'ALLENMORGAN')]


# jk1 =  outmerge4a[(outmerge4a['YEAR'] == 2016)]
...
for wyear in range(2021, 2022, 1): 
    ...
    # sum donation to the same committee
    outmerge2 = outmerge1.groupby(['YEAR','NAME', 'NAME_EXEC', 'NAMEL', 'NAMEF', 'CONAME', 'CONAME_EXEC', 'GVKEY', 'CMTE_ID'])['TRANSACTION_AMT'].sum().reset_index()
    # drop duplicates
    outmerge2 = outmerge2.drop_duplicates(subset=['YEAR', 'NAME_EXEC', 'NAMEL', 'NAMEF', 'CONAME_EXEC', 'CMTE_ID'], keep='first')
    # check how many unique companies donated
    ...
    df_cnccl.dropna(subset = ['CMTE_ID'], inplace = True)      
...
outmerge4a['DMR'] = outmerge4a['TRANSACTION_AMT'] * outmerge4a['CAND_PTY_AFFILIATION'] 
outmerge6a = outmerge4a.groupby(['YEAR','NAME_EXEC', 'NAMEL', 'NAMEF',  'CONAME_EXEC', 'GVKEY'])['TRANSACTION_AMT'].sum().reset_index()
outmerge6b = outmerge4a.groupby(['YEAR','NAME_EXEC', 'NAMEL', 'NAMEF',  'CONAME_EXEC', 'GVKEY'])['DMR'].sum().reset_index()

# DMR is difference between donation to D candidate and R candidates, Total_AMT is the sum of donation to D and R cnadidates
outmerge6b['PoliticalView'] = outmerge6b['DMR'] / outmerge6a['TRANSACTION_AMT']  
outmerge6b['Total_AMT'] = outmerge6a['TRANSACTION_AMT']


outmerge5b = outmerge5b.append([outmerge6b], ignore_index=True)


outmerge5b.to_csv('C:/Users/d_huang/Documents/PythonProjects/Execucompdata/' + 'TOP5view2' + '.csv')
